...
--    + HW feedback/Launchpad MK2: map Launchpad MK2
  ...
  ---------------------------------------------------------------------  
  function DATA2:Clip_Play_BuildScene(trID, sceneID, current_scene) 
    local ret,proj_ptr = DATA2:LiveProject_Validate(true) if not ret then return end
    ...
    local scenepos = DATA2.live.scenes[current_scene].pos_ST
    local sceneend = DATA2.live.scenes[current_scene].pos_EN 
    ...
    GetSetMediaItemInfo_String( item, 'P_EXT:MPLSESVIEW_ITEM_SCENECURRENT', current_scene, true ) -- refer to defined scene
    UpdateArrange()
  ...
  ---------------------------------------------------------------------  
  function DATA2:Clip_Play_RefreshScene(trID, sceneID, force_refresh, current_scene) 
    local ret,proj_ptr = DATA2:LiveProject_Validate(true) if not ret then return end
    if not (DATA2.live.scenes and 
            DATA2.live.scenes[sceneID] and
            DATA2.live.tracks and 
    ...
    for itemidx = cntit, 1, -1 do
      ...
      local hasscene, current_sceneIDext =DATA2:LiveProject_ReadContent_ItemExtState(it,'MPLSESVIEW_ITEM_SCENECURRENT')
      if isclip and isslave and hasscene and sceneIDext == sceneID and current_scene==current_sceneIDext then 
    ...
    if not exist or force_refresh then
      DATA2:Clip_Play_BuildScene(trID, sceneID, current_scene)  
      UpdateArrange()
    end
  ...
  ---------------------------------------------------------------------  
  function DATA2:Clip_Play_Enable(trID, sceneID, pos, disable, currentID) 
    local ret,proj_ptr = DATA2:LiveProject_Validate(true) if not ret then return end
    if not (DATA2.live.scenes and 
            DATA2.live.scenes[sceneID] and
            DATA2.live.tracks and 
    ...
    for itemidx = cntit, 1, -1 do
      ...
      local isclip = DATA2:LiveProject_ReadContent_ItemExtState(it,'MPLSESVIEW_ITEM_ISCLIP')
      local isslave =DATA2:LiveProject_ReadContent_ItemExtState(it,'MPLSESVIEW_ITEM_SLAVE')
      local hasscene, sceneID0 =DATA2:LiveProject_ReadContent_ItemExtState(it,'MPLSESVIEW_ITEM_SCENE')
      local hasscene, sceneIDext =DATA2:LiveProject_ReadContent_ItemExtState(it,'MPLSESVIEW_ITEM_SCENECURRENT')
      if isclip and isslave and hasscene and sceneID0 == sceneID and sceneIDext == currentID then 
  ...
  function DATA2:Scenes_play_initfromstop(proj_ptr, sceneID,donottriggerfollowedscened)
    ...
    for trID in pairs(DATA2.live.tracks) do
      if DATA2.live.tracks[trID].scenes then
        for sceneID2 in pairs(DATA2.live.tracks[trID].scenes) do
          DATA2:Clip_Play_Enable(trID, sceneID2, pos, true, sceneID) 
        end
      end
    end
  ...
  function DATA2:Clip_Play(trID, sceneID, allowtriggerscene) 
    ...
    if not currentID then currentID = sceneID end
    
    -- trigger from stop
      local playstate = DATA2.project_var.playstate
      if playstate == false and allowtriggerscene then  
        DATA2:Clip_Play_RefreshScene(trID, sceneID, nil, currentID) 
    ...
    -- schedule while playing
      ...
      if state == 0 and currentID and currentID == sceneID then 
      ...
      if state == 0 and currentID and currentID ~= sceneID then 
        DATA2:Clip_SetState(trID, sceneID, 1)
        DATA2:RefreshLights()
        local first_enabled_pos = DATA2:Clip_Play_Enable(trID, sceneID, DATA2.project_var.playpos, currentID)  
        if first_enabled_pos then 
          DATA2.sheduled_actions[#DATA2.sheduled_actions+1] = {pos = first_enabled_pos, func = function() 
            DATA2:Clip_SetState(trID, sceneID, 2, true) 
            DATA2:Clip_Play_Enable(trID, sceneID) 
          ...
          DATA.GUI.buttons['grid_tr_'..trID..'_scene_'..sceneID..'L2back'].onmousereleaseR = function() DATA:GUImenu(
            {
              ...
              func = function () DATA2:Clip_Play_RefreshScene(trID, sceneID, true, sceneID) end
              }
            }
          ) end
        end
