  version = '0.21 // 14.04.24'
  UI_title = 'SubtReader' 
  
  -- Color items by exact same name
...
------------------------------------------------------------------------------------------------  
  ...
  function DATA:Color_ByText(inputtxt)
    ...
    for i = 1, #DATA.subt do
      if translit2english(DATA.subt[i].note):match(inputtxt)  then reaper.SetMediaItemInfo_Value( DATA.subt[i].itptr, 'I_CUSTOMCOLOR', color|0x1000000) end
    end 
    UPD.ui = true
  ...
  function DATA:Color_Artist(mode, artist_t)
    ...
    if mode == 0 then 
      if not DATA.curnote_Id then return end
      curnote = DATA.curnote_Id
      curartist = translit2english(DATA.subt[curnote].artist )
    end
    ...
    for i = 1, #DATA.subt do
      ...
      if mode == 1 then
        for j=1, #artist_t do
          ...
          if artist == DATA.subt[i].artist then 
  ...
  function _Main()
    ...
    -- parse ext
      ...
      local fp_conf = filename:gsub('%.lua','_conf.txt'):gsub('_ColByArtist','')
